...
void llevarAlPrincipio(unsigned int pos) {
		...
		int i = 1; Nodo *nodo, *ptr = _prim;
		
		if ((pos <= 0) || (pos > longitud())) throw EPosicionInvalida();
		if (esVacia()) throw EColaVacia();//no es del todo necesario

		if (pos >= 2){//si la posicion es superior a dos debemos mover los elementos 

			while (i < pos - 1){
				ptr = ptr->_sig;//apuntamos al elemento anterior al que queremos mover
				++i;
			}

			nodo = ptr->_sig; // ya tenemos el puntero del elemento que queremos mover
			ptr->_sig = nodo->_sig; // El anterior de la pos apunta al siguiente de la pos
			if (pos == _nelems) _ult = ptr;// caso de mover el ultimo elemento 
			nodo->_sig = _prim; // el siguiente nodo a la pos es el primero
			_prim = nodo;//el primer elemento empiza en en nodo
