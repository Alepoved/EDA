...
private:
	...
	*/
	...
	public:
		...
		Nodo(int n,const T &elem) : _info1(n),_info2(elem), _sig(NULL), _ant(NULL) {}

		Nodo(Nodo *ant,int n, const T &elem, Nodo *sig) :
			_info1(n), _info2(elem), _sig(sig), _ant(ant) {}
...
public:
	...
	*/
	...
	void sumaUno(const T &elem) {

		/*Nodo *ini = _prim;
		while (ini->_info2 != elem) {
			ini = ini->_sig;
		}
		Nodo *aux = ini;
		if (aux != _prim) {
			ini->_ant->_sig = ini->_sig;
			ini->_sig->_ant = ini->_ant;
		}
		while (ini->_sig->_info1 < (aux->_info1) + 1) {
			ini = ini->_sig;
		}
		if (ini != _prim) {
			if (aux == _prim)_prim = aux->_sig;
			aux->_sig = ini->_sig;
			ini->_sig = aux;
			aux->_ant = ini;
		...
		while (aux2->_sig->_info1 < aux->_info1 + 1)
			aux2 = aux2->_sig;
	...
	void splice(Lista<char> l2,char v1,char v2,char v3) {
		
		Nodo *nodo1 = _prim;
		Nodo *nodo2 = l2._prim;

		while (nodo1->_elem != v1) { 
			nodo1 = nodo1->_sig; }

		while (nodo2->_elem != v2) {
			nodo2 = nodo2->_sig; }

		Nodo *fin2 = nodo2;
		Nodo *fin1 = nodo1->_sig;//fin1 = d
		...
		nodo2->_ant->_sig = fin2->_sig;
		fin2->_sig->_ant = nodo2->_ant;
