...
private:
	...
	*/
	...
	public:
		...
		Nodo(Nodo *ant,int n, const T &elem, Nodo *sig) :
			_elem(elem), _info1(n),_sig(sig), _ant(ant) {}
	...
	*/
	static Nodo *insertaElem(int n,const T &e, Nodo *nodo1, Nodo *nodo2) {
		Nodo *nuevo = new Nodo(nodo1,n, e, nodo2);
...
public:
	...
	void CambiarPosiciones() {
		...
		int i = 0, mitad = _numElems / 2;
		Nodo *ini = _prim, *fin = _ult;
		Nodo *auxIni = NULL, *auxFin = NULL;
	...
	void enredar() {
		...
		Nodo *ini = _prim; Nodo *fin = _ult;
		Nodo *iniAux = _prim->_sig; Nodo *finAux = _fin->_ant;

		int mitad = _numElems / 2; int i = 0;
		...
		/*Nodo *auxIni = _prim; //Comenzamos en el primero
		Nodo *auxUlt = _ult; //Comenzamos al final
		Nodo *miNodo = auxIni->_sig;
		Nodo *auxAnt;

		//BUCLE
		int mitad = _numElems / 2, i;

		if ((_numElems % 2 != 0)) i = 0; //-> impar 
		else i = 1; // par 

		while (i < mitad) {
			auxUlt->_sig = miNodo; //Siguiente al ultimo es el siguiente del primero(miNodo)
			auxAnt = auxUlt->_ant;
			auxIni->_sig = auxUlt; //Siguiente al primero es el ultimo
			miNodo = auxUlt->_sig; //miNodo es la segunda posicion, ahora tercera al meter entremedias el ultimo

								   //Actualizaciones de Punteros
			auxUlt = auxUlt->_ant; //El ultimo ahora es el penultimo original
			auxIni = miNodo;
			miNodo = auxIni->_sig;
			auxAnt->_sig = NULL; //El anterior al ultimo inicial es NULL para evitar problemas al liberar
			_ult = auxAnt;

			//Ampliacion de indice i
			++i;
		}*/
	}
	...
	*/
	void pon_ppio(int n,const T &elem) {
		_prim = insertaElem(n,elem, NULL, _prim);
		_numElems++;
	...
	*/
	void pon_final(int n,const T &elem) {
		_ult = insertaElem(n,elem, _ult, NULL);
		_numElems++;
		if (_prim == NULL)
			_prim = _ult;
	}
	...
	*/
	void insertar(int n,const T &elem, const Iterator &it) {
		if (_prim == it._act)
			pon_ppio(n,elem);
		else if (it._act == NULL)
			pon_final(n,elem);
		else {
			insertaElem(n,elem, it._act->_ant, it._act);
			_numElems++;
		}
	}
